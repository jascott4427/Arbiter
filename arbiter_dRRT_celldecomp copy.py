...
TIME_HORIZON = GRID_SIZE * 5 / SPEED  # Total time horizon in seconds
...
def split_lines_at_vertices(valid_edges, vertices):
    """
    Split lines into smaller segments if they intersect any vertex.
    :param valid_edges: List of LineString objects representing valid edges.
    :param vertices: List of vertices (x, y) in the grid.
    :return: List of LineString objects representing split segments.
    """
    split_segments = []
    tol = 0.025  # Tolerance for vertex-edge intersection

    # Helper function to check if two segments are equal (avoid duplicates)
    def segments_equal(seg1, seg2):
        return (seg1.equals(seg2) or (seg1.equals(LineString([seg2.coords[1], seg2.coords[0]]))))

    for edge in valid_edges:
        # Get the start and end points of the edge
        start_point = edge.coords[0]
        xs, ys = start_point
        end_point = edge.coords[1]
        xe, ye = end_point
        # Collect all vertices that lie on the edge (excluding the endpoints)
        intersecting_vertices = []
        for vertex in vertices:
            x, y = vertex
            if vertex != start_point and vertex != end_point:  # Exclude endpoints
                if not (abs(x-xs)<tol and abs(y-ys)<tol) and not (abs(x-xe)<tol and abs(y-ye)<tol):
                    # Create a Point object for the vertex
                    vertex_point = Point(vertex)
                    # Check if the vertex lies on the edge
                    distance = edge.distance(vertex_point)
                    if distance < tol:  # Check if vertex lies on the edge within tolerance
                        intersecting_vertices.append(vertex)

        # Sort the intersecting vertices along the edge
        if intersecting_vertices:
            # Project vertices onto the edge and sort them
            sorted_vertices = sorted(intersecting_vertices, key=lambda v: edge.project(Point(v)))

            # Add the segment from the start point to the first intersecting vertex
            first_segment = LineString([start_point, sorted_vertices[0]])
            if not any(segments_equal(first_segment, existing_segment) for existing_segment in split_segments):
                split_segments.append(first_segment)
                print(f"Added initial segment: {list(first_segment.coords)}")

            # Add segments between intersecting vertices
            for i in range(len(sorted_vertices) - 1):
                segment = LineString([sorted_vertices[i], sorted_vertices[i + 1]])
                if not any(segments_equal(segment, existing_segment) for existing_segment in split_segments):
                    split_segments.append(segment)
                    print(f"Added segment: {list(segment.coords)}")

            # Add the segment from the last intersecting vertex to the end point
            final_segment = LineString([sorted_vertices[-1], end_point])
            if not any(segments_equal(final_segment, existing_segment) for existing_segment in split_segments):
                split_segments.append(final_segment)
                print(f"Added final segment: {list(final_segment.coords)}")
        else:
            # If no intersecting vertices, keep the original edge
            if not any(segments_equal(edge, existing_segment) for existing_segment in split_segments):
                split_segments.append(edge)
                print(f"No intersecting vertices for edge: {list(edge.coords)}")
...
def trapezoidal_decomposition(walls, degrees=60):
    ...
    def project_point(point):
        x, y = point
        # Project the point onto the sweep line
        projected_x = x * np.cos(sweep_line_angle) + y * np.sin(sweep_line_angle)
        return projected_x

    # Sort vertices based on their projection onto the sweep line
    ...
    for i, vertex in enumerate(vertices):
        ...
        tol = inc  # Tolerance threshold for splitting

        # Compute distances from (x, y) to (x1, y1) and (x2, y2)
        dist1 = np.hypot(x1 - x, y1 - y)
        dist2 = np.hypot(x2 - x, y2 - y)

        if dist1 > inc / tol or dist2 > inc / tol:
    ...
    #         if is_valid:
    #             # Split the valid edges at intersecting vertices
    #             valid_edges.append(line)

    # Form one polygon of grid
    polygon = Polygon([(0,0), (0,GRID_SIZE), (GRID_SIZE,GRID_SIZE), (GRID_SIZE,0)])
    
    # Combine all obstacles into a single MultiPolygon
    obstacles_union = unary_union(walls)

    # Subtract the obstacles from the grid polygon
    free_space = MultiPolygon([polygon]).difference(obstacles_union)

    # Ensure the result is a MultiPolygon (in case the difference results in multiple polygons)
    if isinstance(free_space, Polygon):
        print('not multi')
        free_space = MultiPolygon([free_space])
        cells = [polygon]
    # for line in segments:
    #     x, y = line.xy
    #     new_cells = []
    #     for poly in cells:
    #         split_line = LineString([(x[0], y[0]), (x[1], y[1])])
    #         split_polygons = split(poly, split_line)

    #         # Convert result into individual polygons and add them to new_cells
    #         new_cells.extend(split_polygons.geoms)  

    #     cells = new_cells  # Update the cell list

    vertices.extend(extra_vertices)
    # split_segments = split_lines_at_vertices(valid_edges, vertices)    
...
class CellDecompFrame(tk.Frame):
    def __init__(self, parent, env):
        super().__init__(parent)
        self.env = env
        self.figure = plt.figure(figsize=(7, 7))
...
class MainWindow(tk.Tk):
    def __init__(self):
        ...
        cell_decomp_frame = CellDecompFrame(self, self.env)
        cell_decomp_frame.pack(side="left", fill="both", expand=True)
